...
# --- Path adjustment for utils modules ---
# Assuming streamlit_app.py is in 'src/dashboard/' relative to your repo root.
# This logic adds 'your-repo/src' to the Python path.
current_file_path = os.path.abspath(__file__)
# Go up from 'streamlit_app.py' to 'dashboard' -> 'src' -> 'your-repo'
repo_root = os.path.abspath(os.path.join(current_file_path, '..', '..', '..'))
src_dir = os.path.join(repo_root, 'src')

if src_dir not in sys.path:
    sys.path.insert(0, src_dir) # Add 'your-repo/src' to the beginning of the path

# --- CORRECTED IMPORTS FOR YOUR UTILS ---
try:
    # Now, if utils is under 'src', these imports should work
    from utils.data_loader import load_segmentation_data, load_prediction_model 
    from utils.business_logic import calculate_segment_metrics 
except ImportError as e:
    st.error(f"❌ Critical Import Error: Could not load utils modules. {e}")
    st.info("Please ensure 'utils/data_loader.py' and 'utils/business_logic.py' exist within your 'src' directory.")
    st.info(f"Attempted to add '{src_dir}' to sys.path.")
    st.stop()
...
# Custom CSS for enhanced UI aesthetics and animations
st.markdown("""
<style>
    /* Import Google Fonts */
    ...
    .launch-page-container {
        ...
        width: 100vw;
        height: 100vh;
        background: linear-gradient(135deg, #a1c4fd 0%, #c2e9fb 100%); /* Light gradient background */
        display: flex;
        flex-direction: column;
        justify-content: center;
        align-items: center;
        z-index: 9999; /* Ensure it's on top */
        transition: all 0.7s ease-out; /* Smooth transition for slide/fade */
        animation: fadeIn 0.5s ease-out; /* Initial fade in */
    }

    /* Animation for launch page disappearing */
    .launch-page-hidden {
        transform: translateY(-100vh); /* Slide up out of view */
        opacity: 0;
        pointer-events: none; /* Disable interactions after animation starts */
    }

    /* Animation for main content appearing */
    ...
    /* Metric cards (used on launch page and dashboards) */
    .metric-card {
    ...
    /* General animations for dashboard elements */
    ...
    .stSelectbox > div > div, .stMultiSelect > div > div, .stNumberInput > div > div > input, .stSlider > div > div > div {
        border-radius: 10px;
        border: 2px solid #e2e8f0;
        transition: all 0.3s ease;
    }
    .stSelectbox > div > div:focus-within, .stMultiSelect > div > div:focus-within, .stNumberInput > div > div > input:focus {
        border-color: #667eea;
    ...
    .stProgress > div > div > div > div { /* Progress bar */
        background: linear-gradient(90deg, #667eea 0%, #764ba2 100%);
    }
</style>
""", unsafe_allow_html=True)

# --- Fallback Data and Model Functions (if real ones fail) ---
...
def create_mock_segment_metrics(customers_df):
    """Create mock segment metrics for demonstration"""
    segment_metrics = {}
    ...
    for segment in customers_df['Customer_Category'].unique():
        ...
        segment_metrics[segment] = {
            ...
            'avg_revenue': np.random.uniform(40, 100),
            'total_revenue': np.random.uniform(10000, 50000),
            'avg_satisfaction': np.random.uniform(2.5, 4.5),
            'avg_risk': np.random.uniform(1, 8),
            'roi': np.random.uniform(100, 400),
            'net_benefit': np.random.uniform(1000, 10000)
        }
    return segment_metrics

# --- Data and Model Loading with Fallbacks ---
@st.cache_data
def get_customers_df():
    """Load customer data, with fallback to sample data if real data fails"""
    try:
        df = load_segmentation_data()
        if df is None or df.empty:
            st.warning("⚠️ 'load_segmentation_data' returned empty/None. Using sample data.")
            return create_sample_data()
        if 'Customer_Category' not in df.columns:
            st.warning("⚠️ 'Customer_Category' column missing in loaded data. Generating sample categories.")
        ...
        st.error(f"❌ Error in 'load_segmentation_data': {e}. Using sample data.")
        return create_sample_data()

@st.cache_resource
def get_model_artifacts():
    """Load model artifacts, with fallback to mock model if real model fails"""
    try:
        ...
        if model is None or scaler is None or feature_columns is None or category_mapping is None:
            st.warning("⚠️ 'load_prediction_model' returned incomplete artifacts. Using mock model.")
            return create_mock_model_artifacts()
        return model, scaler, feature_columns, category_mapping
    except Exception as e:
        st.error(f"❌ Error in 'load_prediction_model': {e}. Using mock model.")
        return create_mock_model_artifacts()

# --- Business Logic with Fallbacks ---
@st.cache_data
def get_segment_metrics(customers_df):
    """Calculate segment metrics, with fallback for missing columns"""
    try:
        metrics = calculate_segment_metrics(customers_df)
        if not metrics: # If calculate_segment_metrics returns empty (e.g., due to missing columns)
            st.warning("⚠️ 'calculate_segment_metrics' returned empty. Generating mock metrics.")
            return create_mock_segment_metrics(customers_df)
        return metrics
    except Exception as e:
        st.error(f"❌ Error in 'calculate_segment_metrics': {e}. Generating mock metrics.")
        return create_mock_segment_metrics(customers_df)


# --- Dashboard Page Functions (from your original code, cleaned and integrated) ---
...
def show_customer_segments(customers_df, segment_metrics):
    ...
    metrics = segment_metrics.get(selected_segment, {}) if segment_metrics else {}
    
    col1, col2 = st.columns(2)
    
    with col1:
        st.subheader(f"📊 {selected_segment} Overview")
        st.metric("Total Customers", f"{metrics.get('count', len(segment_data)):,}")
        st.metric("Avg Monthly Revenue", f"USD {metrics.get('avg_revenue', segment_data.get('Total_Charges', pd.Series([0])).mean()):.2f}")
    ...
    with col2:
        ...
        st.metric("Avg Satisfaction Score", f"{metrics.get('avg_satisfaction', segment_data.get('Satisfaction_Score', pd.Series([0])).mean()):.2f}")
        st.metric("ROI", f"{metrics.get('roi', 150):.1f}%")
        st.metric("Avg Risk Score", f"{metrics.get('avg_risk', segment_data.get('Risk_Score', pd.Series([0])).mean()):.2f}")
...
def show_launch_page():
    # Use a container to apply the animation class for the launch page
    st.markdown('<div id="launch-page-wrapper" class="launch-page-container">', unsafe_allow_html=True)
    
    st.markdown('<div class="main-header" style="color: #4CAF50;">🚀 Welcome to Smart Customer Segmentation Platform</div>', unsafe_allow_html=True)
    ...
    col1, col2, col3, col4 = st.columns(4)
    with col1: st.markdown("""<div class="metric-card"><h2>🎯</h2><h3>94.46%</h3><h5>Model Accuracy</h5><p>AI-driven segmentation</p></div>""", unsafe_allow_html=True)
    with col2: st.markdown("""<div class="metric-card"><h2>💰</h2><h3>2,653%</h3><h5>ROI Achievement</h5><p>Exceptional returns</p></div>""", unsafe_allow_html=True)
    with col3: st.markdown("""<div class="metric-card"><h2>⚡</h2><h3>Real-time</h3><h5>Instant Analysis</h5><p>Lightning-fast insights</p></div>""", unsafe_allow_html=True)
    with col4: st.markdown("""<div class="metric-card"><h2>🔮</h2><h3>Predictive</h3><h5>Future-Ready</h5><p>Advanced ML models</p></div>""", unsafe_allow_html=True)
    
    st.markdown("---")
    st.markdown("<h3>Click to launch the platform!</h3>", unsafe_allow_html=True)
    
    if st.button("🎉 Launch Platform Experience!", use_container_width=True, type="primary"):
        # Add the 'launch-page-hidden' class to trigger the animation
        st.markdown(
            """
            <script>
                document.getElementById('launch-page-wrapper').classList.add('launch-page-hidden');
            </script>
            """,
            unsafe_allow_html=True
        )
        st.session_state.show_launch = False
        time.sleep(0.7) # Wait for animation to play before rerunning
        st.rerun()

    st.markdown('</div>', unsafe_allow_html=True) # Close the launch-page-container
...
if __name__ == "__main__":
    ...
    # If launch page is active, display it.
    if st.session_state.show_launch:
        show_launch_page()
    else:
        # Once launch page is dismissed, the main content appears
        # We don't need a specific 'main_content_style' session state here
        # as the main content will naturally fade in due to default Streamlit rendering
        # after the launch page element is removed from the DOM.
        main()
